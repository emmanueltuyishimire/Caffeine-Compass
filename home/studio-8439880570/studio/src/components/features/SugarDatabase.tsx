...
export default function SugarDatabase() {
  ...
  const filteredDrinks = useMemo(() => {
    ...
    switch (sortOption) {
      case 'sugar_asc':
        filtered.sort((a, b) => (a.sugar ?? Infinity) - (b.sugar ?? Infinity));
        break;
      case 'name_asc':
      ...
      case 'name_desc':
        ...
        break;
      case 'calories_desc':
        filtered.sort((a, b) => (b.calories ?? -1) - (a.calories ?? -1));
        break;
      case 'calories_asc':
        filtered.sort((a, b) => (a.calories ?? Infinity) - (b.calories ?? Infinity));
        break;
      case 'sugar_desc':
      default:
        filtered.sort((a, b) => (b.sugar ?? -1) - (a.sugar ?? -1));
        break;
    }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Input
                    ...
                    aria-label="Search for a drink by name"
                />
              </div>
              <div>
                <Label htmlFor='sort-by'>Sort by</Label>
                 <Select value={sortOption} onValueChange={(value) => setSortOption(value as SortOption)}>
                  <SelectTrigger id='sort-by' className="mt-1" aria-label="Sort drinks">
          ...
          <div className="p-1 sm:p-4" role="list">
            {filteredDrinks.length > 0 ? (
              filteredDrinks.map((drink) => {
                const Icon = categoryIcons[drink.category as keyof typeof categoryIcons] || PlusCircle;
                ...
                return (
                    <div
                        key={drink.id}
                        role="listitem"
                        className="flex items-center justify-between p-2 sm:p-3 rounded-lg hover:bg-muted group"
                    >
                        ...
                        <div className="text-right flex items-center gap-2 sm:gap-4">
                            ...
                            <div>
                                <p className="font-bold text-primary text-base sm:text-lg">{drink.sugar === undefined ? 'N/A' : `${drink.sugar}g Sugar`}</p>
                                <p className="text-xs sm:text-sm text-muted-foreground">{drink.calories === undefined ? 'N/A' : `${drink.calories} Calories`}</p>
                            </div>
                        </div>
                    </div>
                )
